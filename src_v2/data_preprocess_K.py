...
class DataPreprocess:
    ...
    def seperate_kpis_labels(self) -> None:
        ...
        self.train_data = None
        self.val_data = None
        print("kpi shape",self.train_kpis.shape)
    ...
    def __one_hot_encode_train(self,current_time,log_path):
        ...
        if self.approach == "new":
            ...
            if "clutter_class_met_stations_WS3_AIRPORT" not in df_encoded.columns.to_list():
                ...
                print("ws3_airport column is missing")
            ...
            if "clutter_class_met_stations_LOW-MEDIUM URBAN" not in df_encoded.columns.to_list():
                ...
                print("lm urban column is missing")
                for i in range(1,self.prev_day_data):
                ...
                if "clutter_class_met_stations_OPEN LAND" not in df_encoded.columns.to_list():
                    ...
                    print("open  land column is missing")
                    for i in range(1,self.prev_day_data):
            ...
            if column in cat_cols:
                ...
                if self.approach == "new":
                    if column in ws3_clutter_columns:
                       ...
                       # print("its in column index for ws3", column_categories)
                    if column in wslmu_clutter_columns:
                        column_names.insert(2,column+"_LOW-MEDIUM URBAN")
                       # print("its in column index for wslmu", column_categories)
                    if column in wsol_clutter_columns:
                        column_names.insert(5,column+"_OPEN LAND")
                        #print("its in column index for wsol", column_categories)   
        ...
        print("this is feature number after encode",self.feature_number)
...
#        p_kpis = tf.tile(kpis, [3, 1, 1])
    ...
    def get_k_nearest_WS(kpis:tf.Tensor,num_weather_stations:int,approach:str) -> tf.Tensor:
        ...
        """
        if approach == "new":
            #kpis=tf.reshape(kpis, [1024,3,4,84])
            transformed_kpis = kpis[:,0:num_weather_stations,:,:]
            #transformed_kpis=tf.reshape(transformed_kpis, [1024,3*4*84])
        elif approach == "prev":
