...
all_data2=np.loadtxt(path2+f'fp_ws_new_data.txt')
print(all_data2.shape)
all_data2 = all_data2.reshape(16,3,4,84)
for i in range(16):
    for j in range(3):
         ws_data.append(all_data2[i][j][0][9])
        #print(all_data[i][j][1][9])
print(len(ws_data))
all_data4=np.loadtxt(path4+f'fp_ws_new_data.txt')
print("4", all_data4.shape)
all_data4 = all_data4.reshape(11,3,4,84)
for i in range(11):
    for j in range(3):
...
for i in range(16):
        ...
        count_all+=1
        shap = np.loadtxt(path2+f'fp{i}_shap_data.txt')
    ...
    # print(shap_values.shape)
        ...
        for i in range(3):
            ws_all_f=0
            for j in range(84):
                ws_all_f+=feature_wise_importance_abs[i][j]
           # for j in range(32):
...
for i in range(11):
    ...
    # label_list = labels.split(",")
       ...
       # data = data.reshape(3,4,84)
        ...
        # print(shap_values.shape)
            feature_wise_importance = shap_values.sum(axis=(0, 1))
            feature_wise_importance_abs = shap_values.sum(axis=(1))
            ws_wise_importance = np.abs(shap_values).sum(axis=(1, 2))
            if feature_wise_importance[3]>0:
                count+=1
            uavail.append(feature_wise_importance[3])
...
plt.scatter(ws_data, ws_feature, color='red', edgecolor='black', alpha=0.8)

# Add labels and title
plt.xlabel('WS Distance')
plt.ylabel('Sum of SHAP Feature values')
plt.title('Scatter Plot of WS Distance vs SHAP Feature values for FP cases')
...
plt.savefig('ws_distance_vs_all_shap_fp_scatterplot.pdf', format='pdf')
