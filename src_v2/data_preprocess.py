    ...
    def __one_hot_encode_train(self,current_time,log_path):
        ...
        def find_and_print_mismatches(df):
            ...
            for i, col in enumerate(columns):
                ...
                if normalized_col not in similar_columns:
                    similar_columns[normalized_col] = []
            ...
            # Print mismatches
            if mismatches:
                print("Columns that are the same except for the 'WS' value but have mismatches:")
                for normalized_col, cols in mismatches.items():
                    print(f"{normalized_col}: {cols}")
            else:
       ...
       # find_and_print_mismatches(df_encoded)
        ...
        for column in df_encoded.columns.unique():
            if "WS3" in column:
                ...
                other+=1  
               # df_13.append(column)
       # print(f"ws3: {ws3_count}, ws5: {ws5_count}, ws4: {ws4_count}, oth: {other},  ws1: {ws1_count}, ws2: {ws2_count}")
...
# Print the counts for verification
       ...
       # print(f"Other count: {len(df_encoded)}")
        ...
        if self.approach == "new":
           ...
           # wslmu_clutter_columns = []
            ws4_clutter_columns = []
            ws5a_clutter_columns = []
            wslmua_clutter_columns = []
            ws1_clutter_columns = []
            ws4a_clutter_columns = []
            for k in {4,5}:
                j=""
                if k == 5:
                    j="WS5"
                    ...
                    '''
                    if f"clutter_class_met_stations_{j}_AIRPORT" not in df_encoded.columns.to_list():
                        df_encoded[f'clutter_class_met_stations_{j}_AIRPORT'] = 0.0
                        for i in range(1,self.prev_day_data):
                            df_encoded[f"clutter_class_met_stations_T-{i}_{j}_AIRPORT"] = 0.0
                            ws5a_clutter_columns= [
                        ...
                        '''
                    if f"clutter_class_met_stations_{j}_LOW-SPARSE URBAN" not in df_encoded.columns.to_list():
                        df_encoded[f'clutter_class_met_stations_{j}_LOW-SPARSE URBAN'] = 0.0
                        for i in range(1,self.prev_day_data):
                            df_encoded[f"clutter_class_met_stations_T-{i}_{j}_LOW-SPARSE URBAN"] = 0.0
                        ws5_clutter_columns = [
                            f"clutter_class_met_stations_{j}",
                            f"clutter_class_met_stations_T-1_{j}",
                            f"clutter_class_met_stations_T-2_{j}",
            ...
            if "clutter_class_met_stations_LOW-SPARSE URBAN" not in df_encoded.columns.to_list():
                    ...
                    print("this is clutter for ws1")
                        
                    '''
                    if "clutter_class_met_stations_OPEN LAND" not in df_encoded.columns.to_list():
                        df_encoded['clutter_class_met_stations_OPEN LAND'] = 0.0
                        for i in range(1,self.prev_day_data):
                            df_encoded[f"clutter_class_met_stations_T-{i}_OPEN LAND"] = 0.0
                        wsol_clutter_columns = [
                            "clutter_class_met_stations",
                            "clutter_class_met_stations_T-1",
                            "clutter_class_met_stations_T-2",
                            "clutter_class_met_stations_T-3",
       ...
       # print(f"Other count before all column: {other}, ws3: {ws3_count}, ws5: {ws5_count}, ws4: {ws4_count}, ws1: {ws1_count}, ws2: {ws2_count}")
        ...
        for column in self.train_kpis.columns.to_list():
            ...
            if column in cat_cols:
                ...
                if self.approach == "new":
                    if column in ws4a_clutter_columns:
                        column_names.insert(0,column+"_AIRPORT")
                    if column in ws5a_clutter_columns:
                        column_names.insert(0,column+"_AIRPORT")
                    if column in wslmua_clutter_columns:
                        column_names.insert(0,column+"_AIRPORT")
                    if column in ws4_clutter_columns:
                        column_names.insert(7,column+"_LOW-SPARSE URBAN")
                    if column in ws5_clutter_columns:
                        column_names.insert(7,column+"_LOW-SPARSE URBAN")
                       # print("I am here for ws5")  
                    if column in ws1_clutter_columns:
                        column_names.insert(7,column+"_LOW-SPARSE URBAN") 
                        #print("I am here for ws1")  
                    #if column in wsol_clutter_columns:
                        #column_names.insert(5,column+"_OPEN LAND")  
    ...
    def __one_hot_encode_val(self,one_hot_encoder):
        ...
        if self.approach == "new":
            ...
            if "clutter_class_met_stations_WS5_LOW-SPARSE URBAN" not in df_encoded.columns.to_list():
                df_encoded['clutter_class_met_stations_WS5_LOW-SPARSE URBAN'] = 0.0
                for i in range(1,self.prev_day_data):
                    df_encoded[f"clutter_class_met_stations_T-{i}_WS5_LOW-SPARSE URBAN"] = 0.0   
