...
# Add missing columns to balance the counts
       ...
       # print(f"Other count: {len(df_encoded)}")
        ...
        joblib.dump(one_hot_encoder,f"/home/papry/Exp_Project/MS_thesis-main/logs/{current_time}/one_hot_encoder_{self.approach}.save")
...
#        p_kpis = tf.tile(kpis, [3, 1, 1])
        ...
        #kpis=tf.reshape(kpis, [3*4*84])   #Add for XAI
        kpis=tf.reshape(kpis, [3*4*80])
        return kpis, labels
    ...
    def get_k_nearest_WS(kpis:tf.Tensor,num_weather_stations:int,approach:str) -> tf.Tensor:
        ...
        """
        if approach == "new":
           # kpis=tf.reshape(kpis, [1024,3,4,84])         #for Expalinability
            kpis=tf.reshape(kpis, [1024,3,4,80])
            transformed_kpis = kpis[:,0:num_weather_stations,:,:]
           # transformed_kpis=tf.reshape(transformed_kpis, [1024,3*4*78])
            transformed_kpis=tf.reshape(transformed_kpis, [1024,3*4*80])  #for Expalinability
        elif approach == "prev":
