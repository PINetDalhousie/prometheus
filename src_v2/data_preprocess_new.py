...
class DataPreprocess:
    ...
    def seperate_kpis_labels(self) -> None:
        ...
        print("train kpis", self.train_kpis.shape)
    ...
    def __min_max_scale_train(self,current_time,log_path):
        ...
        # Save scaler
        joblib.dump(min_max_scaler,f"/users/grad/papry/MS_thesis-main/logs/{current_time}/min_max_scaler_{self.approach}.save")
    ...
    def __normalize_train(self,current_time,log_path):
        ...
        # Save scaler
        joblib.dump(normalizer,f"/users/grad/papry/MS_thesis-main/logs/{current_time}/normalize_{self.approach}.save")
    ...
    def __one_hot_encode_train(self,current_time,log_path):
        ...
        def normalize_column_name(column_name):
            if "WS" in column_name:
                return re.sub(r'_WS\d+', '', column_name)
            return column_name
       ...
       # find_and_print_mismatches(df_encoded)
        Missing_columns=["AIRPORT", "AVERAGE-DENSE URBAN", "BUILTUP-VILLAGE", "DENSE TREE",
                      "GREEN HOUSE", "INDUSTRIAL & COMMERCIAL", "LOW-DENSE URBAN", "LOW-MEDIUM URBAN",
                        "LOW-SPARSE URBAN", "OPEN IN URBAN", "OPEN LAND", "SEA", "SPARSE TREE"]

        missing_columns_list = list(Missing_columns)
        print("Missing columns", missing_columns_list)
        ...
        for column in df_encoded.columns.unique():
            if "WS3" in column:
                ws3_count+=1
                if "clutter_class_met_stations_" in column:
                    words=column.split("_")
                    if words[-1] in Missing_columns and words[-1] not in df_32:
                        df_32.append(words[-1])
                ...
                if "clutter_class_met_stations_" in column:
                    words=column.split("_")
                    if words[-1] in Missing_columns and words[-1] not in df_52:
                        df_52.append(words[-1])
            elif "WS4" in column:
                ws4_count+=1 
                if "clutter_class_met_stations_" in column:
                    words=column.split("_")
                    if words[-1] in Missing_columns and words[-1] not in df_42:
                        df_42.append(words[-1])
            elif "WS6" in column:
                ws6_count+=1
                if "clutter_class_met_stations_" in column:
                    words=column.split("_")
                    if words[-1] in Missing_columns and words[-1] not in df_62:
                        df_62.append(words[-1])
            elif "WS2" in column:
                ws2_count+=1 
                if "clutter_class_met_stations_" in column:
                    words=column.split("_")
                    if words[-1] in Missing_columns and words[-1] not in df_22:
                        df_22.append(words[-1])
                ...
                if "clutter_class_met_stations_" in column:
                    words=column.split("_")
                    if words[-1] in Missing_columns and words[-1] not in df_12:
                        df_12.append(words[-1])
        print(f"ws3: {ws3_count}, ws5: {ws5_count}, ws4: {ws4_count}, oth: {other},  ws6: {ws6_count}, ws2: {ws2_count}", len(Missing_columns))
        # Get mismatches with corresponding indices
        mismatches_1 = [(indx, col) for indx, col in enumerate(Missing_columns) if col not in df_12]
        mismatches_2 = [(indx, col) for indx, col in enumerate(Missing_columns) if col not in df_22]
        mismatches_3 = [(indx, col) for indx, col in enumerate(Missing_columns) if col not in df_32]
        mismatches_4 = [(indx, col) for indx, col in enumerate(Missing_columns) if col not in df_42]
        mismatches_5 = [(indx, col) for indx, col in enumerate(Missing_columns) if col not in df_52]
        mismatches_6 = [(indx, col) for indx, col in enumerate(Missing_columns) if col not in df_62]

        # Print mismatches with corresponding indices
        print("for weather st 1", mismatches_1)
        print("for weather st 2", mismatches_2)
        print("for weather st 3", mismatches_3)
        print("for weather st 4", mismatches_4)
        print("for weather st 5", mismatches_5)
        #print("for weather st 6", mismatches_6)

        # Unpack indices and column names into separate lists
        id_1, col_1 = zip(*mismatches_1) if mismatches_1 else ([], [])
        id_2, col_2 = zip(*mismatches_2) if mismatches_2 else ([], [])
        id_3, col_3 = zip(*mismatches_3) if mismatches_3 else ([], [])
        id_4, col_4 = zip(*mismatches_4) if mismatches_4 else ([], [])
        id_5, col_5 = zip(*mismatches_5) if mismatches_5 else ([], [])
        id_6, col_6 = zip(*mismatches_6) if mismatches_6 else ([], [])

        
       # max_count = max(ws3_count, ws4_count, ws5_count, ws1_count, ws2_count)
        '''
        print("for weather st 1")
        find_and_print_2(df_12, df_22)
        print("for weather st 6")
        find_and_print_2(df_62, df_22)
        print("for weather st 3")
        find_and_print_2(df_32, df_22)
        print("for weather st 4")
       # find_and_print_mismatches(df_34)
        find_and_print_2(df_42, df_22)
        print("for weather st 5")
        find_and_print_2(df_52, df_22)
       # find_and_print_mismatches(df_35)
       '''
        
# Function to add missing columns

        def add_missing_columns(df, prefix, count, max_count):
            for i in range(count, max_count):
                df[f'{prefix}_missing_{i}'] = 0.0
            return df

# Add missing columns to balance the counts
       # df_encoded = add_missing_columns(df_encoded, 'WS3', ws3_count, max_count)
        #df_encoded = add_missing_columns(df_encoded, 'WS4', ws4_count, max_count)
        #df_encoded = add_missing_columns(df_encoded, 'WS5', ws5_count, max_count)
       # df_encoded = add_missing_columns(df_encoded, '', other, max_count)
        #df_encoded = add_missing_columns(df_encoded, 'WS2', ws2_count, max_count)


                
       # print(f"Other count: {len(df_encoded)}")
        ...
        if self.approach == "new":
           ...
           # wslmu_clutter_columns = []
            ...
            ws6_clutter_columns = []
            ws_clutter_columns = {}
            for k in range(1, self.num_stations+1):
                if k != 1:
                    j = f"WS{k}"
                    col_var = eval(f"col_{k}")
                    for m in col_var:
                    
                        print( f"clutter_class_met_stations_{j}_{m}")
                        if f"clutter_class_met_stations_{j}_{m}" not in df_encoded.columns.to_list():
                            df_encoded[f"clutter_class_met_stations_{j}_{m}"] = 0.0
                            for i in range(1,self.prev_day_data):
                                df_encoded[f"clutter_class_met_stations_T-{i}_{j}_{m}"] = 0.0
                               
                    if len(col_var)>0:
                        ws_clutter_columns[f"ws{k}_clutter_columns"]= [
                                f"clutter_class_met_stations_{j}",
                                f"clutter_class_met_stations_T-1_{j}",
                                f"clutter_class_met_stations_T-2_{j}",
                                f"clutter_class_met_stations_T-3_{j}"
                            #  "clutter_class_met_stations_T-4_{j}"
                            ]
                        print(ws_clutter_columns[f"ws{k}_clutter_columns"])
                else:
                    col_var = eval(f"col_{k}")
                    
                    for m in col_var:
                    
                        print( f"clutter_class_met_stations_{m}")
                        if f"clutter_class_met_stations_{m}" not in df_encoded.columns.to_list():
                            df_encoded[f"clutter_class_met_stations_{m}"] = 0.0
                            for i in range(1,self.prev_day_data):
                                df_encoded[f"clutter_class_met_stations_T-{i}_{m}"] = 0.0
                    if len(col_var)>0:
                        ws1_clutter_columns= [
                                f"clutter_class_met_stations",
                                f"clutter_class_met_stations_T-1",
                                f"clutter_class_met_stations_T-2",
                                f"clutter_class_met_stations_T-3"
                            #  "clutter_class_met_stations_T-4_{j}"
                            ]
        ...
        cat_ws=0
        for column in df_encoded.columns.unique():
            if "clutter_class" in column:
                cat_ws+=1
        print(f"cat_ws: {cat_ws}")
        ...
        print(f"Other count before all column: {other}, ws3: {ws3_count}, ws5: {ws5_count}, ws4: {ws4_count}, ws6: {ws6_count}, ws2: {ws2_count}")
        # Call __calc_feat_number with balanced train_kpis
        #find_and_print_mismatches(df_35)
        
        #find_and_print_mismatches(df_45)
                ...
                if self.approach == "new":
                    for k in range(1, self.num_stations+1):
                        if k != 1:
                            col_var = eval(f"col_{k}")
                            id_var = eval(f"id_{k}")
                            for i, j in zip(id_var, col_var):
                             if column in ws_clutter_columns[f"ws{k}_clutter_columns"]:
                                column_names.insert(i,column+f"_{j}")
                               # for i in range(1,self.prev_day_data):
                                    #column_names.insert(0,column+f"_T-{i}_{j}")
                        else:
                            col_var = eval(f"col_{k}")
                            id_var = eval(f"id_{k}")
                            for i, j in zip(id_var, col_var):
                                if column in ws1_clutter_columns:
                                    column_names.insert(i,column+f"_{j}")
                                                 
                     
                order_preserved_columns.extend(column_names)
        ...
        # Balance the number of columns for each category
        ...
        # Update train_kpis with balanced df_encoded
        ...
        print(f"Other count before all column: {other}, ws3: {ws3_count}, ws5: {ws5_count}, ws4: {ws4_count}, ws1: {ws1_count}, ws2: {ws2_count}")
        # Call __calc_feat_number with balanced train_kpis
        find_and_print_mismatches(df_35)
        max_count = max(ws3_count, ws4_count, ws5_count, other, ws2_count)
        
        def add_missing_columns(df, prefix, count, max_count):
            for i in range(count, max_count):
                df[f'{prefix}_missing_{i}'] = 0.0
                column= f'{prefix}_missing_{i}'
                self.order_preserved_columns.extend([column]) 
            return df
      ...
      #  df_encoded = add_missing_columns(df_encoded, 'WS5', ws5_count, max_count)
        ...
        self.train_kpis = df_encoded[self.order_preserved_columns]
        self.__calc_feat_number(self.train_kpis)
...
# Add missing columns to balance the counts
       ...
       # print(f"Other count: {len(df_encoded)}")
        ...
        for column in self.test_kpis.columns.to_list():
            ...
            if column in cat_cols:
                ...
                if self.approach == "new":
                    for k in range(1, self.num_stations+1): #6 is for k=5
                        if k != 1:
                            col_var = eval(f"col_{k}")
                            id_var = eval(f"id_{k}")
                            for i, j in zip(id_var, col_var):
    ...
    def scale_test_data(self) -> None:
        ...
        """
        scaler = joblib.load(f"/users/grad/papry/MS_thesis-main/scalers/min_max_scaler_{self.approach}.save")
    ...
    def pad_missing_catgorical_columns(self,approach):
        ...
        with open(f'/users/grad/papry/MS_thesis-main/data/{approach}.txt', 'r') as f:
            lines = f.readlines()
    ...
    def positional_encoding(kpis, labels):
        ...
        positional_tensor = tf.tile(positional_tensor, [5, 1, 1])
#        p_kpis = tf.tile(kpis, [3, 1, 1])
        ...
        print("after positional", kpis.shape)
        #kpis=tf.tile(kpis, [3, 1, 1])
        #kpis=tf.reshape(kpis, [5*4*82])   #Add for XAI
        return kpis, labels
    ...
    def pad_to_batch_size(
        ...
        if approach == "new":
            ...
            # Add upper line for XAI and remove below one 
    ...
    def get_k_nearest_WS(kpis:tf.Tensor,num_weather_stations:int,approach:str) -> tf.Tensor:
        ...
        """
        if approach == "new":
           # kpis=tf.reshape(kpis, [1024,5,4,82])         #for Expalinability
            transformed_kpis = kpis[:,0:num_weather_stations,:,:]
           # transformed_kpis=tf.reshape(transformed_kpis, [1024,5*4*82])  #for Expalinability
        elif approach == "prev":
