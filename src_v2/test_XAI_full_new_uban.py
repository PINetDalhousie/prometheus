...
def main():
    ...
    parser.add_argument(
        '--model_path',
        type=str,
        default="/home/papry/Exp_Project/MS_thesis-main/logs/20251021_012848/ckpt",
        help='Path to trained model'
        )  
    parser.add_argument(
        '--log_path',
        type=str,
        default="/home/papry/Exp_Project/MS_thesis-main/logs/20251021_012848/",
    ...
    for kpis, labels in test_ds:
        ...
        for i in range(len(labels)):
            if labels[i][1]>0.5:
               # print("pdata", i)
                pc=pc+1
                failure_set.append(kpis[i])
                lbl.append(i)
            else:
                non_failure_set.append(kpis[i])
       ...
       # for i in range (len(lbl)):
        ...
        def fn(z):
           ...
           # print("inside fn", z.shape)
            pred=[]
            #inputs=tf.reshape(inputs,[1,3,4,78])
...
# compute SHAP values
           ...
           # print("masker",masker.data.shape)
            ...
            explainer = shap.SamplingExplainer(fn, data_set) 
...
#Extract false negative cases from test data
            ...
            for i in range (len(failure_set)):
              ...
              #  z=tf.reshape(z, [1,1008]) #for transformer model
                z=tf.reshape(z, [1,3*4*78]) #for explainability lstm
                z1=z
                
              #  print("shape of z1", z1.shape)
               # z1=tf.reshape(z1, [1,3,4,84])
                z1=tf.reshape(z1, [1,3,4,78])
                z=z._numpy()
                z1=z1._numpy()  
                pred=fn(z)
              # for i in range(len(batch1)):
...
#Extract false positive cases from test data
            ...
            print("Total failure", len(failure_set))
            for i in range (len(failure_set)):
                indx=i
              #  print("index", indx)
                z=failure_set[indx]
               ...
               # z=tf.reshape(z, [1,1008])
                z=tf.reshape(z, [1,3*4*fs]) #for explainability lstm
                z1=z
                
              #  print("shape of z1", z1.shape)
                #z1=tf.reshape(z1, [1,3,4,84])
                z1=tf.reshape(z1, [1,3,4,fs])
                z=z._numpy()
                z1=z1._numpy()  
                pred=fn(z)
              # for i in range(len(batch1)):
...
# Extract Shapley values from the explainer
            ...
            #sv=sv._numpy()
                ...
                #z=z.reshape((1,3,4,84))
                  ...
                  sv1=sv[0]
                  #sp.append(sv1)
                  #with open(args.log_path+f'input{indx}_shap_data.txt', 'w') as outfile:
                   # for slice_2d in sv1:
