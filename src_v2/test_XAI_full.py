...
def main():
    ...
    parser.add_argument(
        '--model_path',
        type=str,
        default="/home/papry/Exp_Project/MS_thesis-main/logs/20251009_005125/ckpt",
        help='Path to trained model'
        )  
    parser.add_argument(
        '--log_path',
        type=str,
        default="/home/papry/Exp_Project/MS_thesis-main/logs/20251009_005125/",
...
#Extract false negative cases from test data
            ...
            for i in range (len(failure_set)):
              ...
              #  z=tf.reshape(z, [1,1008]) #for transformer model
                z=tf.reshape(z, [1,3*4*79]) #for explainability lstm 78
                z1=z
                
              #  print("shape of z1", z1.shape)
               # z1=tf.reshape(z1, [1,3,4,84])
                z1=tf.reshape(z1, [1,3,4,79])
                z=z._numpy()
                z1=z1._numpy()  
               # pred=fn(z)
              # for i in range(len(batch1)):
          ...
          #  print("Total fp", total_fp)
            ...
            for i in range (len(failure_set)):
              ...
              #  print("index", indx)
                z=failure_set[indx]
                #print("shape of z", z.shape)
                z=tf.reshape(z, [1,3*4*79]) #for explainability lstm
                z1=z
                ws.append(z1[0])
              #  print("shape of z1", z1.shape)
               # z1=tf.reshape(z1, [1,3,4,84])
            ...
            #sv=sv._numpy()    
                ...
                #z=z.reshape((1,3,4,84))
                  ...
                  with open(args.log_path+f'shap{indx}_new_data.txt', 'w') as outfile:
                    for slice_2d in sv1:
...
# make a standard partial dependence plot
    ...
    with open(args.log_path+f'fp_ws_new_data.txt', 'w') as outfile:
                    for slice_2d in ws:
                        np.savetxt(outfile, slice_2d) 
